# SORCH 仕様書（新構想・単一SNN + Actuation Drivers）
...
補足（Phase 1 のベンチ）:
- `python -m sorch.bench.latency_bench` は JSONL と同名の `*.meta.json` を出力し、計測条件（frames/sample_rate等）を保存する。
- `--stop-mode close_reopen` を使うと、出力ストリームの close/reopen を含む“より物理寄り”の停止タイミングも記録できる（デバイスやOSバッファの影響は別途）。
...
## 9.4 Phase 2（短期記憶/オンライン更新）の運用ルール

### 状態ベクトル（Readout入力）の扱い
- ベンチでは `state_mode` を明示して比較し、採用案を固定する
- 次元が重い場合は Random Projection とセットで決める

### RLS（オンライン更新）の安定運用
- 実運用を意識する場合、更新間隔は 10–50ms 相当まで落としてよい
  - ただし「間引き」ではなく、サンプルをまとめて更新する（block update）を優先
- 忘却（`lam<1`）は不安定化しやすいので、まずは `lam=1.0` を基準にする（ベンチで根拠を残す）
...
- outputs/
  - phase1/latency/（JSONL + meta）
  - phase1/report/（集計レポート）
  - phase2/mc/runs/, phase2/mc/reports/
  - phase2/rls/runs/, phase2/rls/reports/

保存方針（推奨）:
- 生ログ/生CSVなど肥大化しやすいものは Git から除外し、Markdownレポートを残す

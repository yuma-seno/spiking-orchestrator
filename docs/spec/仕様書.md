...
| **中枢層** | **Orchestrator Core** | **【開発の核心】** 以下の領域による協調制御。<br>1. **Reflex Circuit** (反射・安全装置)<br>2. **Hippocampal Reservoir** (短期記憶・文脈)<br>3. **Orchestrator Executive**（呼び出し制御・割り込み方針・相槌などを学習で最適化） |
...
* **役割:** 割り込みの“兆候”を低遅延に検知し、必要なら即時停止（安全装置）する。
* **特性:** 重み固定。誤検知を防ぐための**不感期（Hold-off）**に加え、**環境適応性**と**自己発話抑制**を持つ。
* **入力:** 多次元特徴量アンサンブル (Envelope, ZCR, Spectral Centroid, Voicing Flag)。
* **出力:**
    * `Hard_Stop_Signal`（安全上の即時停止: Motorへ最優先で伝搬）
    * `Interrupt_Cue`（会話上の割り込み兆候: Coreの能動判断の入力として使う。これ自体は停止命令ではない）

補足（重要）:
- **Hard Stop と Soft Interrupt（割り込み兆候）は分離する。**
    - Hard Stopは安全上の理由で最優先（例: 明示的停止、危険状態、ハウリング等）。
    - Soft Interruptは「ユーザが話し始めた/割り込みたい」等の兆候であり、**実際に“止める/話し切る/待ってと言う”はCoreが状態と文脈を見て判断**する。
...
補足（意図）:
- Hippocampal Reservoirは「会話の流れ」を保持し、Coreが能動的に判断するための状態を提供する。
- 例:
    - 入力やRWKV出力（要約/意図など）を短期記憶し、同一/類似の指示が繰り返されたときは **思考層を呼ばずに高速応答（Fast Path）** する
    - 応答生成の待ち時間に、状況に応じた **相槌（バックチャネル）** を適切なタイミングで挿入する

### 3.3. Orchestrator Executive（呼び出し・割り込み・相槌の制御）
Orchestrator Coreは、反射（Reflex）と短期記憶（Reservoir）を入力として、
「思考層をいつ呼ぶか」「運動層へいつ何を出すか」「割り込みにどう応じるか」を**能動的に選択**する。

原則（重要）:
- ここは“ルールのハードコーディング”を増やしすぎない。
- 代わりに、SNNが学習で適応できるように **入力（観測）と出力（行動）と評価（ログ/KPI）** の環境を整える。

入力（観測）の例:
- `Reflex_Output`（Hard_Stop/Interrupt_Cue など）
- `Semantic_Vector`（ユーザ入力・直近履歴・RWKV出力の要約/意図など）
- `Motor_State`（再生中/キュー残量/残チャンク/現在の重要度/中断コストなど、後述）

出力（行動）の例:
- `Cognitive_Request`（RWKV等を呼ぶ/呼ばない、温度/最大トークン等の要求）
- `Speech_Plan`（本文 + 低次元メタ）
- `Interrupt_Action`（例: 今は話し切る / 一旦停止して譲る / 「ちょっと待って」を返す）

注記:
- Hard Stopは安全上の最優先であり、Executiveの判断をバイパスしてMotorへ伝搬してよい（最優先経路）。
- Soft InterruptはExecutiveの入力であり、**状況（残り1語/重要説明/誤検知の可能性等）を含めて学習で最適化**する。

### 3.4. Orchestrator → Motor の出力インターフェース（Speech Plan）
...
1. **入力: Speech Plan**
    ...
    - Reflexからの `Hard_Stop_Signal` は最優先で割り込み処理される（安全装置）。
    - Reflexからの `Interrupt_Cue`（割り込み兆候）は、原則としてCore Executiveの判断に入力される（Motor単独で即停止しない）。

1.1 **出力（フィードバック）: Motor State**
    - Coreが能動的に割り込み方針（話し切り/譲る/待って）を学習できるよう、Motorは最低限の再生状態をCoreへ返す。
    - 例: 再生中フラグ、キュー残量（チャンク数/推定ms）、現在チャンクの残り、発話の重要度ラベル、停止コスト見積もり等。
...
* **Step 3.2: マルチプロセス・アーキテクチャの構築**
    * **内容:** PythonのGIL回避のため、`multiprocessing` を用いてプロセス分離を行う。
        ...
        2.  **Core Process:** SNN/Reflex Logic（Reflex Circuit / Hippocampal Reservoir / Orchestrator Executive を含む）。

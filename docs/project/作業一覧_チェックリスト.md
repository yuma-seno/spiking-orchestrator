...
- [ ] 作成したコマンド/オプションを [docs/ガイド.md](../%E3%82%AC%E3%82%A4%E3%83%89.md) の「すぐ動かす」「コマンド」節に追加
	- 状況: Phase 2 の追加オプション（state_mode/projなど）が増えている。
	- 次: `mc_project` 系の主要オプションをガイドに追記し、手順書への導線も併記
- [ ] 行った作業の成果を [docs/project/成果サマリ.md](../project/%E6%88%90%E6%9E%9C%E3%82%B5%E3%83%9E%E3%83%AA.md) に反映
	- 状況: Step 2.3 のレポートと暫定結論が未反映。
	- 次: Step 2.3（state_mode×投影）の結論とレポートリンクを成果サマリに追記
- [ ] ベンチマーク等を行ったら、結果レポートを outputs/phase2/ 配下に保存
	- 状況: MC探索/Readout比較は outputs/phase2/mc/reports/ に保存済み
	- 次: RLS（Step 2.4）の結果は outputs/phase2/rls/ に保存する（runs/reportsを分ける）
...
- [ ] Readoutが参照する状態ベクトル設計（どの信号を含めるか）
	...
	- [ ] 採用する `state_mode`（必要なら `proj_out_dim`）を決定して、手順書/仕様書へ反映
		- 状況: repeats5の結果は揃っており、軽さ優先の現実案は **`state_mode=v+spike` + `proj_out_dim=200`**。
		- 次: これを「Phase2の標準設定」として明記し、今後のベンチはこの設定を基本にする
...
- [x] オンライン学習ベンチ雛形（RLSでreadout更新し、遅延ごとのR^2を出す）（`sorch/bench/rls_online.py`）
	- 状況: 実装は追加済み。スモーク実行と結果のレポート化が未。
	- 次: seedを変えてrepeatsし、`mc_online` と delay別R^2が安定するか確認
- [ ] オンライン学習ベンチ（ReadoutをRLSで更新し、遅延/安定性を確認）
	- 状況: オフライン（ridge）でのMC評価は整備済み。オンライン版の評価が未。
	- 次: 更新間隔（10–50ms）と `lam/delta` をスイープできるベンチを追加
- [ ] 更新間隔（10–50ms）での安定動作（発散しない／性能が維持できる）
	- 状況: ベンチ未実装のため未評価。
	- 次: まずは1遅延（例: delay=20）での追従を安定させ、その後multi-delayへ拡張

...
- [ ] Action Plan の最小スキーマを決める（Orchestrator→Motorの契約）
	- 原則: 項目を増やさず、区分管理を避ける（固定長ベクトル + 任意テキスト）
	- 例: `text` + 低次元制御ベクトル `u(t) \in R^K`
	- 用語: Action Plan（[用語集](%E7%94%A8%E8%AA%9E%E9%9B%86.md#glossary-action-plan)）

- [ ] Motor Cortex側で Action Plan（u）→ TTSパラメータのマッピングを実装
...
- [ ] Motor → Core の状態フィードバックI/F（学習の観測）を定義
	...
	- 将来（マルチモーダル）: 実行中アクション種別（音声/操作/ロボット等）、安全停止状態、実行失敗理由なども含める
- [ ] Soft Interrupt（割り込み兆候）のイベント定義（Hard Stopと分離）
	- 用語: Soft Interrupt（[用語集](%E7%94%A8%E8%AA%9E%E9%9B%86.md#glossary-soft-interrupt)）

- [ ] Reflex/Reservoirの「単一SNN統合」方針を決める（やる/やらない、段階導入）
	- 前提: **Hard Stop（安全停止）は統合の有無に関係なく独立に保護**（最優先経路でMotorへ直通）
	- 用語: 単一SNN統合（[用語集](%E7%94%A8%E8%AA%9E%E9%9B%86.md#glossary-single-snn-integration)）/ 領域分割（[用語集](%E7%94%A8%E8%AA%9E%E9%9B%86.md#glossary-region-partitioning)）
	- 目的: 表現力を上げつつ、時間スケール衝突や学習干渉で安全性/KPIが崩れないようにする
	- 成果物（最低限）:
		- 統合案のアブレーション表（分離/入力だけ共有/部分結合/完全統合）
		- 回帰項目（Stopレイテンシ、FP/FN、Soft Interruptの方針品質）

- [ ] Motorをマルチモーダルに拡張する方針を決める（上位統合 + 下位Driver分離）
	- 目的: 音声だけでなく、マウス/キーボード/ロボット等の出力を同じ枠組み（優先度/キャンセル/安全停止）で扱う
	- 用語: Action Plan（[用語集](%E7%94%A8%E8%AA%9E%E9%9B%86.md#glossary-action-plan)）/ モダリティ（[用語集](%E7%94%A8%E8%AA%9E%E9%9B%86.md#glossary-modality)）/ Driver（[用語集](%E7%94%A8%E8%AA%9E%E9%9B%86.md#glossary-driver-adapter)）
	- 前提: **Hard Stop（安全停止）は最優先で、学習/通常通信をバイパスして止められること**
...
- [ ] Soft Interrupt（割り込み兆候）に対する割り込み方針を学習で最適化する
	...
	- [ ] 統合の有無でHard StopのKPIが悪化しないことを回帰で担保する
		- 例: publish→検知（無音化）/ publish→物理停止（close相当）のp95/p99が退行していない

...
- [ ] ベンチ実行手順を確認: [docs/howto/Phase1_遅延計測ベンチ_手順.md](../howto/Phase1_%E9%81%85%E5%BB%B6%E8%A8%88%E6%B8%AC%E3%83%99%E3%83%B3%E3%83%81_%E6%89%8B%E9%A0%86.md)
- [ ] 計測条件（frames/sample-rate等）を記録できるようにする

### Step 1.1: Reflex（暫定DSP→将来SNN置換）
- [ ] 割り込み兆候（Soft Interrupt）を観測として定義
- [ ] 誤検知/チャタリングの回帰観測（ログ）を整備

### Step 1.2: Hard Stop を“実デバイス停止”までE2E検証
- [ ] publish→無音化 と publish→物理停止 を分けて計測・保存
- [ ] Driverの停止セマンティクス（無音化/停止/close/キャンセル）を明文化
...
### Step 2.1: STP リザーバ
- [ ] STPの採用範囲（どの領域/どの状態を観測するか）を仕様化

### Step 2.2: MC 探索
- [ ] 実行手順を確認: [docs/howto/Phase2_MC探索_手順.md](../howto/Phase2_MC%E6%8E%A2%E7%B4%A2_%E6%89%8B%E9%A0%86.md)
- [ ] 探索結果の保存方針（Git管理/非管理）を決める

### Step 2.3: 状態ベクトル設計と次元削減
- [ ] state vector / readout の責務分担を仕様化（Core内・Driver内の境界）

### Step 2.4: RLS 等でオンライン更新
- [ ] 更新間隔（10–50ms相当）での安定運用条件を仕様として固定

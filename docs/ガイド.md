...
- 設計方針: Orchestrator→Motorは **Action Plan（任意テキスト + 低次元制御ベクトル）** を渡し、詳細な表現はMotor側の実装で表現する
	- 目的: Core側のI/O項目を増やさず、学習を複雑にしすぎない（区分管理を避ける）
	- 仕様: [docs/spec/仕様書.md](spec/%E4%BB%95%E6%A7%98%E6%9B%B8.md) の「Orchestrator → Motor（Action Plan）」および「運動層仕様」を参照

将来目標（マルチモーダル出力）:
- 出力が音声だけでなく、マウス/キーボード/ロボット等に広がっても、Core→MotorのI/Fは **Action Planのまま** で拡張できるようにする。
- 考え方は「上位の実行制御（優先度/キャンセル/安全停止）は統合」「下位のデバイス制御はモダリティ別Driverに分離」。

### 2.6 Core（反射 + 短期記憶 + Executive）
Coreは「止める」だけの回路ではなく、
反射（Reflex）と短期記憶（Reservoir）を使って **能動的に判断**する“司令塔（Executive）”です。

やりたいこと（例）:
- 思考層（RWKV等）を「呼ぶ/呼ばない」を状況に応じて選ぶ（Fast Path）
- 応答待ちの間に、適切な相槌（バックチャネル）を入れて会話を自然に繋ぐ
- 割り込みを「即停止」だけでなく、発話の残り/重要度に応じて話し切りや“ちょっと待って”を選ぶ

重要な整理:
- **Hard Stop**（安全停止）: 最優先で必ず止める
- **Soft Interrupt**（割り込み兆候）: Coreの判断材料（止める/話し切る/待つ等は学習で最適化）

補足（よくある疑問）: Reflex と Reservoir は単一SNNにまとめるべき？
- **まとめてもOK**です（Reflex領域 / Reservoir領域 / Executive を「概念として」分けられていれば良い）。
- ただし **Hard Stop は“統合の外”で守る**のが前提です。
	- 理由: 学習や通常通信の混雑で安全停止が遅れるのが最悪パターンだから
	- 具体: 最優先経路（共有フラグ等）でMotorへ直通させる
- 進め方のおすすめは「分離で測る→統合を段階導入→KPI回帰で担保」です。
	- 統合で表現力は上がり得ますが、時間スケール衝突や学習干渉が起きやすいので、まず“測れて壊れない”土台が重要です。
